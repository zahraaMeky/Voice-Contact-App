...
class Ui_MainWindow(QMainWindow):
    ...
    def checkInfo(self):
        ...
        # print(self.running)
        if (self.running):
            self.info.label_ip_connection.setText("متصل")
        else:
            self.info.label_ip_connection.setText("غير متصل")
        
    def check_if_client_connected(self):
        check, toned = self.receiver.check_client()
        # print("check client : " , check , " " , toned)
        ...
        if (check):
            ...
            # if (not self.speaking_status):
            #     self.speak_timer = QTimer(self)
            #     self.speak_timer.timeout.connect(self.StartSpeaking)
            #     self.speak_timer.start(2000)
        else:
            # if (self.speaking_status):
            #     self.nospeak_timer = QTimer(self)
            #     self.nospeak_timer.timeout.connect(self.StopSpeaking)
            #     self.nospeak_timer.start(2000)
            self.speaking(False)
            self.speaking_status = False

        # if(check):
        #     self.label_streaming_status.setText("يتحدث الان...")
        # else:
        #     self.label_streaming_status.setText("مغلق")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_6.setText(_translate("MainWindow", "ّIP العنوان"))
